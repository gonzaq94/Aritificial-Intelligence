...
### The game_smell.py describes the simulation (smell)
import game_smell
...
env = game_smell.PyRat() ####change
exp_replay = rl_non_linear_smell.ExperienceReplay(max_memory=max_memory)
model = rl_non_linear_smell.NNModel(env.observe()[0],"model_smell")
...
def play(model,epochs,train=True):
    ...
    for e in tqdm(range(epochs)):
        ...
        if train:
            ...
            loss_.append(loss)
            wins_.append(win_cnt)
            draw_.append(draw_cnt)
            losses_.append(lose_cnt)
            epochs_.append(e)
        else:
            loss = 0
...
print("Testing done")
np.save("loss_smell.npy",loss_)
np.save("wins_smell.npy",wins_)
np.save("draw_smell.npy",draw_)
np.save("losses_smell.npy",losses_)
np.save("epochs_smell.npy",epochs_)
